...
@farmer_bp.route("/report/sales", methods=["POST"])
@jwt_required()
def generate_sales_report():
    """
    Generate sales report for the farmer based on a date range and format (PDF/CSV).
    """
    try:
        user_id = get_jwt_identity()
        farmer = Farmer.query.get(user_id)
        ...
        orders = (
            ...
            .filter(
                OrderItem.product_id.in_([p.id for p in farmer.products]),
        ...
        if format == "csv":
            csv_output = BytesIO()
            writer = csv.DictWriter(csv_output, fieldnames=sales_data[0].keys())
            writer.writeheader()
...
@farmer_bp.route("/report/inventory", methods=["POST"])
@jwt_required()
def generate_inventory_report():
    """
    Generate inventory report for the farmer in CSV format.
    """
    try:
        user_id = get_jwt_identity()
        farmer = Farmer.query.get(user_id)
        ...
        # Generate CSV
        csv_output = BytesIO()
        writer = csv.DictWriter(csv_output, fieldnames=inventory_data[0].keys())
        writer.writeheader()
